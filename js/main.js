class Kimchi {
    ...
    calcularIngredientes(a, b) {
        ...
        ingredientes.push({
            id: 1,
            ingrediente: "Sal kosher ",
            cantidad: Math.round(nuevaSalKosher),
        });
        let nuevaAguaPasta = (this.UsuarioHakusai * this.aguaPasta) / this.hakusai;
        ingredientes.push({
            id: 2,
            ingrediente: "Agua para la pasta",
            cantidad: Math.round(nuevaAguaPasta),
        });
        let NuevaHarinaArroz =
            (this.UsuarioHakusai * this.harinaArroz) / this.hakusai;
        ingredientes.push({
            id: 3,
            ingrediente: "Harina de arroz ",
            cantidad: Math.round(NuevaHarinaArroz),
        });
        let NuevaAzucarPasta =
            (this.UsuarioHakusai * this.azucarPasta) / this.hakusai;
        ingredientes.push({
            id: 4,
            ingrediente: "Azucar para la pasta",
            cantidad: Math.round(NuevaAzucarPasta),
        });
        let nuevaZanahoria = (this.UsuarioHakusai * this.zanahoria) / this.hakusai;
        ingredientes.push({
            id: 5,
            ingrediente: "Zanahoria",
            cantidad: Math.round(nuevaZanahoria),
        });
        let nuevoNabo = (this.UsuarioHakusai * this.nabo) / this.hakusai;
        ingredientes.push({
            id: 6,
            ingrediente: "Nabo",
            cantidad: Math.round(nuevoNabo),
        });
        let nuevaCebollaVerdeo =
            (this.UsuarioHakusai * this.cebollaVerdeo) / this.hakusai;
        ingredientes.push({
            id: 7,
            ingrediente: "cebolla de verdeo",
            cantidad: Math.round(nuevaCebollaVerdeo),
        });
        let nuevoAjo = (this.UsuarioHakusai * this.ajo) / this.hakusai;
        ingredientes.push({
            id: 8,
            ingrediente: "Ajo",
            cantidad: Math.round(nuevoAjo),
        });
        let nuevoJengibre = (this.UsuarioHakusai * this.jengibre) / this.hakusai;
        ingredientes.push({
            id: 9,
            ingrediente: "jengibre",
            cantidad: Math.round(nuevoJengibre),
        });
        let NuevaCebolla = (this.UsuarioHakusai * this.cebolla) / this.hakusai;
        ingredientes.push({
            id: 10,
            ingrediente: "Cebolla",
            cantidad: Math.round(NuevaCebolla),
        });
        let nuevaSalsaPescado =
            (this.UsuarioHakusai * this.salsaPescado) / this.hakusai;
        ingredientes.push({
            id: 11,
            ingrediente: "salsa de pescado",
            cantidad: Math.round(nuevaSalsaPescado),
        });
        let nuevoGochuGaru = (this.UsuarioHakusai * this.gochuGaru) / this.hakusai;
        ingredientes.push({
            id: 12,
            ingrediente: "Gochugaru",
            cantidad: Math.round(nuevoGochuGaru),
        });
...
function validarFormulario(e) {
    ...
    if (parseInt(inputUsuario)) {
        ...
        $("#resultadoCalc").append(
            "<h6 class='reset'>ERROR:INGRESE UN NUMERO</h6>"
        );
        $(".reset")
            .css({
                color: "red",
...
function calcularTuReceta(input) {
    ...
    tusIngredientes.forEach((item, index) => {
        ...
        //Jquery
        $("#resultadoCalc").prepend(
            `<li><p>Necesitas de  ${item.ingrediente} : ${item.cantidad} gramos </p>`
        );
        $("#resultadoCalc").css({
            "margin-top": "12px",
...
$("#footer-animacion").append(
    '<img src="media/cute-pepper.png" width="150" height="150" class= "happyPepper" alt="haciendo-kimchi"></img>',
